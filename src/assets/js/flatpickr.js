...
function Flatpickr(element, config) {
  ...
  function init() {
    ...
    if (self.config.weekNumbers) {
      self.calendarContainer.style.width =
        self.days.clientWidth + self.weekWrapper.clientWidth + "px";
    }
  ...
  function setHours(hours, minutes, seconds) {
    ...
    if (!self.config.time_24hr && self.selectedDates.length)
      self.amPM.textContent =
        self.latestSelectedDateObj.getHours() >= 12 ? "PM" : "AM";
  ...
  function build() {
    ...
    self.numInputType =
      navigator.userAgent.indexOf("MSIE 9.0") > 0 ? "text" : "number";
    ...
    (customAppend ? self.config.appendTo : window.document.body).appendChild(
      self.calendarContainer
    );
  }
  ...
  function buildDays(year, month) {
    ...
    self.prevMonthDays = self.utils.getDaysinMonth(
  ...
  function buildTime() {
    ...
    self.minuteElement.value = self.pad(
      self.latestSelectedDateObj
    ...
    if (self.config.enableSeconds) {
      ...
      self.secondElement.value = self.latestSelectedDateObj
        ? self.pad(self.latestSelectedDateObj.getSeconds())
        : "00";
  ...
  function destroy(instance) {
    ...
    if (instance.timeContainer)
      instance.timeContainer.removeEventListener(
        "transitionend",
        positionCalendar
      );
    ...
    if (instance.mobileInput && instance.mobileInput.parentNode)
      ...
      instance.calendarContainer.parentNode
    )
      instance.calendarContainer.parentNode.removeChild(
        instance.calendarContainer
      );
  ...
  function formatDate(frmt, dateObj) {
    ...
    return chars
      .map(function (c, i) {
        return self.formats[c] && chars[i - 1] !== "\\"
  ...
  function isEnabled(date, timeless) {
    ...
    for (var i = 0, d; i < array.length; i++) {
      ...
      if (d instanceof Function && d(dateToCheck))
        ...
        typeof d === "string" &&
        self.parseDate(d, true).getTime() === dateToCheck.getTime()
      )
  ...
  function onMouseOver(e) {
    ...
    for (
      var timestamp = self.days.childNodes[0].dateObj.getTime(), i = 0;
      i < 42;
  ...
  function open(e) {
    if (self.isMobile) {
      ...
      self.isOpen ||
      (self.altInput || self.input).disabled ||
      self.config.inline
    )
      return;
    ...
    if (userConfig.altInput && userConfig.enableTime && !userConfig.altFormat) {
      self.config.altFormat = self.config.noCalendar
        ...
        : Flatpickr.defaultConfig.altFormat +
          (" h:i" + (self.config.enableSeconds ? ":S" : "") + " K");
    }
  ...
  function setupLocale() {
    if (
      _typeof(self.config.locale) !== "object" &&
  ...
  function positionCalendar(e) {
    ...
    var calendarHeight = self.calendarContainer.offsetHeight,
      ...
      distanceFromBottom =
        window.innerHeight - inputBounds.bottom + input.offsetHeight,
      bottomCalendar = distanceFromBottom < calendarHeight + 60;

    var top =
      window.pageYOffset +
      inputBounds.top +
      (!bottomCalendar ? input.offsetHeight + 2 : -calendarHeight - 2);
  ...
  function setupHelperFunctions() {
    self.utils = {
      ...
      monthToStr: function monthToStr(monthNumber, shorthand) {
        shorthand =
          typeof shorthand === "undefined"
            ? self.config.shorthandCurrentMonth
            : shorthand;

        return self.l10n.months[(shorthand ? "short" : "long") + "hand"][
          monthNumber
        ];
      },
    };
  ...
  function setupMobile() {
    ...
    if (self.selectedDates.length) {
      self.mobileInput.defaultValue = self.mobileInput.value = formatDate(
        self.mobileFormatStr,
        self.selectedDates[0]
      );
    }
  ...
  function updateNavigationCurrentMonth() {
    ...
    self.currentMonthElement.textContent =
      self.utils.monthToStr(self.currentMonth) + " ";
  ...
  function timeWrapper(e) {
    ...
    if (e.type !== "input" || e.target.value.length === 2) {
      if (newValue < min) {
        ...
        newValue =
          e.target === self.hourElement ? newValue - max - !self.amPM : min;
      }
